...
import static org.linda.common.utils.Utils.formatLine;
import static org.linda.common.utils.Utils.getNode;
import static org.linda.common.utils.Utils.getTx;
...
import static org.linda.core.config.Parameter.CommonConstant.ADD_PRE_FIX_BYTE_MAINNET;
import static org.linda.core.config.Parameter.CommonConstant.ADD_PRE_FIX_BYTE_TESTNET;
import static org.linda.keystore.StringUtils.char2Byte;
import static org.linda.keystore.Wallet.decrypt2PrivateBytes;
...
import org.linda.trident.proto.Common;
...
public class WalletApi {
  ...
  private boolean processTransactionExtention(Response.TransactionExtention transactionExtention)
  ...
  public boolean createAssetIssue(byte[] ownerAddress, String name, String abbrName,
                                  long totalSupply, int lindNum, int icoNum, int precision,
    ...
    Response.TransactionExtention transactionExtention = apiCli.createAssetIssue(ownerAddress, name,
        abbrName, totalSupply, lindNum, icoNum, startTime, endTime, url, freeNetLimit,
  ...
  public boolean exchangeTransaction(byte[] owner, long exchangeId, byte[] tokenId, long quant,
                                     long expected) throws CipherException, IOException, CancelException, IllegalException {
    if (!isUnlocked()) {
      throw new IllegalStateException(LOCK_WARNING);
    }
    if (owner == null) {
  ...
  public byte[] generateContractAddress(byte[] ownerAddress, Chain.Transaction lind) {
    // get tx hash
  ...
  public boolean updateSetting(byte[] owner, byte[] contractAddress,
                               long consumeUserResourcePercent) throws IOException, CipherException, CancelException, IllegalException {
    if (!isUnlocked()) {
      throw new IllegalStateException(LOCK_WARNING);
    }
    if (owner == null) {
  ...
  public static org.linda.trident.api.GrpcAPI.NumberMessage getReward(byte[] owner) {
